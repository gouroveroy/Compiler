...
statement returns [string text]
    ...
    | PRINTLN LPAREN id=ID LTHIRD index=expression RTHIRD RPAREN SEMICOLON
    {
        // Print statement for array element output
        $text = "printf(" + $id->getText() + "[" + $index.text + "]);";
        hasPrintln = true;  // Mark that println is used (for procedure inclusion)
        auto sym = symbolTable->LookUp($id->getText());
        ...
        tempCodeFile << "    CALL PRINT_OUTPUT\n";
        tempCodeFile << "    CALL NEW_LINE\n";
        writeIntoParserLogFile("Line " + to_string($PRINTLN->getLine()) + ": statement : PRINTLN LPAREN ID LTHIRD expression RTHIRD RPAREN SEMICOLON\n\n" + $text + "\n");
        // Check if variable is declared and is printable
        if (!sym) {
            writeIntoErrorFile("Error at line " + to_string($PRINTLN->getLine()) + ": Undeclared variable " + $id->getText() + "\n");
        } else if (sym->getDataType() != "array") {
            writeIntoErrorFile("Error at line " + to_string($PRINTLN->getLine()) + ": Cannot print non-array type " + sym->getDeclaredType() + "\n");
        }
    }
